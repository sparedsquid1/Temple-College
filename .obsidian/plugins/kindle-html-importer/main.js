...
var MyPlugin = class extends import_obsidian.Plugin {
  ...
  async handleFileLoad(fileContents) {
    if (!fileContents)
      return;
    const $2 = load(fileContents);
    const bookTitle = $2(".bookTitle").text().trim().replace(/[\\/*<>:|?"]/g, "");
    const author = $2(".authors").text().trim();
    author.replace(/[\\/*<>:|?"]/g, "");
    let content = "";
    let highlightsCounter = 0;
    $2(".noteHeading").each((index2, element) => {
      if ($2(element).children("span").length !== 1)
        return;
      const pageMatch = $2(element).text().match(/(Page|Location) (\d+)/);
      const pageNumber = pageMatch ? pageMatch[2] : null;
      const noteText = $2(element).next(".noteText").text().trim();
      content += `${noteText}
- ${pageMatch ? pageMatch[1] : ""} ${pageNumber || ""}

`;
      if ($2(element).next().next().children("span").length === 0 && !$2(element).next().next().hasClass("sectionHeading") && $2(element).next().next().length !== 0) {
        const userNote = $2(element).next().next().next(".noteText").text().trim();
        content += `>[!${userNote}] 
...
${content}`
      ...
      new import_obsidian.Notice("File created");
    } catch (error) {
      if (error.code === "ENOENT") {
        new import_obsidian.Notice("Invalid path. Please select a valid folder in the plugin settings");
      } else {
...
var KindleHighlightsSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    const folders = this.app.vault.getAllLoadedFiles().filter(
      (file) => this.app.vault.getAbstractFileByPath(file.path) instanceof import_obsidian2.TFolder
    ).map((folderFile) => folderFile.path);
    new import_obsidian2.Setting(containerEl).setName("File path").setDesc("Select the folder where you want to save your highlights").addDropdown((dropdown) => {
      dropdown.addOptions({
        ...folders.reduce(
          (acc, cur) => ({ ...acc, [cur]: cur }),
          {}
